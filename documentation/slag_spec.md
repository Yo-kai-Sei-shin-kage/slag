...
| `particle.init()`               | Create the 2 GPU particle compute shaders (Emit, Simulate). Idempotent; call once after `gpu.init()` |
| `particle.step(max,params)`     | Run the GPU particle system: dead-pool of `max` particles; `params` = 128-byte EmitterConstants (32 f32). Emit recycles+spawns, Simulate integrates (gravity/wind/drag/curl-turbulence/buoyancy), ages/fades, writes billboard verts. Drawn automatically in the present frame (after geometry, before Present; straight-alpha, depth-read). Data-driven: smoke/fire/sparks/dust/debris differ only by `params`. Needs ≥1 `fill_triangle_gpu` item in the frame to present |
| `particle.read(dest,count)`     | Copy `count` render vertices (64 B each) from the GPU buffer to `dest` so Slag can inspect what Simulate wrote (pos3 uv2 rgba slice flag nrm3 pad2); particle `i` occupies verts `i*6..i*6+5`. Diagnostic |
...
| 0.18.0  | **GPU particle system.** Data-driven GPU-resident particle effects via `particle.init`/`particle.step`: a dead-pool compute sim (Emit recycles+spawns from a 128-byte EmitterConstants block; Simulate integrates gravity/wind/drag/curl-turbulence/buoyancy, ages/fades, and writes camera-facing billboard vertices into a GPU buffer). Drawn automatically inside the present frame (after opaque geometry, before Present; straight-alpha blend, depth-read-only, CULL_NONE). One shader drives smoke/fire/sparks/dust/debris — behavior is entirely the parameter block (buoyancy, turbScale/turbFreq, wind, prewarm, emitRate spawn ramp, gravity, life, size growth). Soft billboards shaped by a texture's alpha; `particle.read` reads render verts back for inspection. **Bug fixes:** particle draw stack-frame prologue/epilogue mismatch (segfault); billboard triangles were back-face culled (valid draw + valid data → zero pixels), fixed with a dedicated CULL_NONE raster state; cold-start pod eliminated by a Slag-ramped `emitRate` (spawns build over time instead of all dead slots filling on frame 1) | ✅ Complete |
...
*Slag Language Specification v0.18.0 — Subject to revision*
