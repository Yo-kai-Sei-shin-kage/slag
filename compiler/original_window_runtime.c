...
static void emit_window_thread_proc(Codegen *cg) {
    ...
    E("    lea  rdx, [rsp]             ; pbmi (our BITMAPINFOHEADER above)");
...
static void emit_window_utils(Codegen *cg) {
    ...
    E("");
    E("    ; b = 5th arg, placed at [rbp+16] by caller (above saved rbp+ret addr)");
    E("    movzx rax, byte [rbp+16]   ; b (5th arg, full qword)");
...
// ---------------------------------------------------------------------
static void emit_default_event_handlers(Codegen *cg) {
    E("; --- default event handler stubs ---");
    E("; User on key_down { } etc. will emit _slag_on_key_down etc.");
    E("; These stubs are only linked if the user provides no handler.");
    E("_slag_on_key_down:");
    E("_slag_on_key_up:");
