...
static void emit_window_constants(Codegen *cg) {
    ...
    E("WSTATE_CLIP_X0     equ 272");
    E("WSTATE_CLIP_Y0     equ 280");
    E("WSTATE_CLIP_X1     equ 288");
    E("WSTATE_CLIP_Y1     equ 296");
    E("WSTATE_SIZE        equ 304");
...
static void emit_window_open(Codegen *cg) {
    ...
    E("    ; default clip = full framebuffer [0,0,width-1,height-1]");
    E("    mov  qword [r15 + WSTATE_CLIP_X0], 0");
    E("    mov  qword [r15 + WSTATE_CLIP_Y0], 0");
    E("    lea  rax, [r12-1]");
    E("    mov  [r15 + WSTATE_CLIP_X1], rax");
    E("    lea  rax, [r13-1]");
    E("    mov  [r15 + WSTATE_CLIP_Y1], rax");
...
static void emit_fill_triangle_pcolor_core(Codegen *cg) {
    ...
    E("    mov  rax, r8");
    E(".ftpc_ys:");
    E("    ; clamp ys up to clip_y0");
    E("    cmp  rax, [rbx + WSTATE_CLIP_Y0]");
    E("    jge  .ftpc_ys_clip");
    E("    mov  rax, [rbx + WSTATE_CLIP_Y0]");
    E(".ftpc_ys_clip:");
    E("    mov  [rbp-456], rax");
    ...
    E("    mov  r12, rax");
    E(".ftpc_ye:");
    E("    ; clamp ye down to clip_y1");
    E("    cmp  r12, [rbx + WSTATE_CLIP_Y1]");
    E("    jle  .ftpc_ye_clip");
    E("    mov  r12, [rbx + WSTATE_CLIP_Y1]");
    E(".ftpc_ye_clip:");
    E("");
    E(".ftpc_scan:");
    E("    mov  rax, [rbp-456]");
    ...
    E(".ftpc_xok:");
    // Clamp x to screen, then to clip rect
    E("    cmp  r13, 0");
    E("    jge  .ftpc_xl");
    E("    xor  r13, r13");
    E(".ftpc_xl:");
    E("    cmp  r13, [rbx + WSTATE_CLIP_X0]");
    E("    jge  .ftpc_xl_clip");
    E("    mov  r13, [rbx + WSTATE_CLIP_X0]");
    E(".ftpc_xl_clip:");
    E("    mov  rax, [rbx + WSTATE_WIDTH]");
    E("    dec  rax");
    E("    cmp  r15, rax");
    ...
    E("    mov  r15, rax");
    E(".ftpc_xr:");
    E("    cmp  r15, [rbx + WSTATE_CLIP_X1]");
    E("    jle  .ftpc_xr_clip");
    E("    mov  r15, [rbx + WSTATE_CLIP_X1]");
    E(".ftpc_xr_clip:");
...
static void emit_window_utils(Codegen *cg) {
    ...
    E("");
    E("    ; reset clip to full framebuffer so next frame starts unclipped");
    E("    mov  qword [rbx + WSTATE_CLIP_X0], 0");
    E("    mov  qword [rbx + WSTATE_CLIP_Y0], 0");
    E("    mov  rax, [rbx + WSTATE_WIDTH]");
    E("    dec  rax");
    E("    mov  [rbx + WSTATE_CLIP_X1], rax");
    E("    mov  rax, [rbx + WSTATE_HEIGHT]");
    E("    dec  rax");
    E("    mov  [rbx + WSTATE_CLIP_Y1], rax");
    E("");
    ...
    // window.clip(x0, y0, x1, y1) - set per-window raster clip rect (pcolor)
    E("; --- _slag_window_clip(rcx=x0, rdx=y0, r8=x1, r9=y1) ---");
    E("_slag_window_clip:");
    E("    push rbp");
    E("    mov  rbp, rsp");
    E("    push rbx");
    E("    push rsi");
    E("    sub  rsp, 32");
    E("    mov  rbx, rcx            ; save x0 (rcx clobbered by TlsGetValue)");
    E("    mov  r10, rdx            ; y0");
    E("    mov  r11, r8             ; x1");
    E("    mov  rsi, r9             ; y1");
    E("    mov  rcx, [_window_tls_index]");
    E("    call TlsGetValue");
    E("    ; rax = struct ptr; sanitize each bound to [0, dim-1]");
    E("    ; x0 = clamp(x0, 0, width-1)");
    E("    mov  rdx, [rax + WSTATE_WIDTH]");
    E("    dec  rdx");
    E("    cmp  rbx, 0");
    E("    jge  .wclip_x0a");
    E("    xor  rbx, rbx");
    ...
    E("    mov  rbx, rdx");
    E(".wclip_x0b:");
    E("    mov  [rax + WSTATE_CLIP_X0], rbx");
    E("    ; x1 = clamp(x1, 0, width-1)");
    E("    cmp  r11, 0");
    E("    jge  .wclip_x1a");
    E("    xor  r11, r11");
    E(".wclip_x1a:");
    ...
    E("    mov  r11, rdx");
    E(".wclip_x1b:");
    E("    mov  [rax + WSTATE_CLIP_X1], r11");
    E("    ; y0/y1 = clamp(., 0, height-1)");
    E("    mov  rdx, [rax + WSTATE_HEIGHT]");
    E("    dec  rdx");
    E("    cmp  r10, 0");
    E("    jge  .wclip_y0a");
    E("    xor  r10, r10");
