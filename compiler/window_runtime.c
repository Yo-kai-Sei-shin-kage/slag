...
static void emit_window_constants(Codegen *cg) {
    ...
    E("FT_ENTRY_SIZE        equ 56     ; xa,ya,xb,yb,xc,yc,color (7 qwords)");
...
static void emit_window_utils(Codegen *cg) {
    ...
    E("    ; split into row bands, one band per worker.");
    E("    mov  r13, [_ft_queue_count]");
    E("    test r13, r13");
    E("    jz   .wf_drain_done");
    ...
    E("    cqo");
    E("    idiv rcx");
    E("    mov  [rbp-16], rax          ; band_height = ceil(height / worker_count)");
    E("");
